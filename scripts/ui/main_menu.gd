...
@onready var collectables_button: Button = %CollectablesButton
...
func _ready() -> void:
	...
	collectables_button.disabled = not StoryCodex.has_any_entries_unlocked()
...
func _on_collectables_pressed() -> void:
	GameManager.open_collectables_screen()
